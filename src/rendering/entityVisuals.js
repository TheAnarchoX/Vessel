(function (root, factory) {
  ...
  root.VesselRendering = Object.assign(root.VesselRendering || {}, api);
  ...
  function drawPixelBody(ctx, x, y, w, h, body, accent) {
    ...
    ctx.fillRect(x - w / 2 + 2, y - h / 2 + 2, Math.max(2, w - 4), Math.max(2, h / 3));
  }
  ...
  function renderPickup(ctx, pickup, nowMs) {
    ...
    ctx.fillRect(pickup.x - 14, pickup.y + 8, 28, 8);
    ctx.fillStyle = "#f2e4bf";
    ctx.fillRect(pickup.x - width / 2, pickup.y - 18, width, height);
    return state;
  }
  ...
  function renderBossPhaseFlash(ctx, boss, flashTimer) {
